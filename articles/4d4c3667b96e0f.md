...
いつの間にか知らない機能が追加されていたり書き方が変わっているthreejsをキャッチアップする目的で、主にexamplesの更新分についてAIの力を借りた要約＆不明な用語・判断を調べながらバージョンアップに伴う各詳細を記録していく。
...
- `Clock` から より高機能な `Timer` クラスに置き換わる。
    - [Timer機能（Clockとの違い）](#timerの機能（clockとの違い）)
...
- 点光源からのゴッドレイ（体積光っぽい光条）をポストプロセスで表現するデモ。
- 外部ライブラリの以下を利用
    - `postprocessing`
    - `three-good-godrays`
...
- WebGPU + TSL で、**PS1風のレトロ画面**をポストプロセスで作るデモ。
...
### [9. USD Kitchen Setが読み込めた - editor](https://t.co/rqspz5lp0g)
[OpenUSDの Kitchen Set ](https://t.co/CqCgWMVi0p)とは、ピクサー（Pixar）が公開している、USD（Universal Scene Description）の機能を網羅的にデモンストレーションするための代表的なサンプルデータセットです。

---

### 10. [新しいアニメーションパネルを追加 - editor](https://t.co/rqspz5lp0g)
