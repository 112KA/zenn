...
- **不要import整理**
    ...
    - `blendColor` をimportから削除し、実装と依存を一致。

---

## r183新規機能・変更点の利用について

このパッチで特にr183要素として言及できるのは次です。

- **`THREE.RenderPipeline` の利用**  
    → r183のWebGPU/ノード系更新に合わせた移行ポイントです。
    
- **SSR出力（premultiplied）前提の合成方式への対応**  
    → `ssrPass.rgb` を加算する形に変更しており、r183でのSSR周辺挙動に合わせた実装です。

---

> Improved Animation Keyframes example 
> [https://threejs.org/examples/#webgl_animation_keyframes](https://t.co/WGAagzP4DU) 

主な改善点は次のとおりです（差分: ocean_r182_r183.patch、対象: webgl_animation_keyframes.html）。

見た目の質向上
THREE.WebGLRendereにtoneMapping = THREE.ACESFilmicToneMapping を設定し、HDR環境光の見え方を改善。
環境ライティングの刷新
...
> Improved Parallax UV example 
> [https://threejs.org/examples/#webgpu_parallax_uv](https://t.co/yeVDHHSVBG) 

主な改善点は次のとおりです（変更対象: ocean_r182_r183.patch, webgpu_parallax_uv.html）。
...
2. **ライトと影**
    ...
    - [PointLight](vscode-file://vscode-app/c:/Users/IizukaMasayuki/AppData/Local/Programs/Microsoft%20VS%20Code/072586267e/resources/app/out/vs/code/electron-browser/workbench/workbench.html) を高強度で配置し、`castShadow = true`。
    - シーン内メッシュを走査して [castShadow/receiveShadow](vscode-file://vscode-app/c:/Users/IizukaMasayuki/AppData/Local/Programs/Microsoft%20VS%20Code/072586267e/resources/app/out/vs/code/electron-browser/workbench/workbench.html) を有効化。
...
- `onWindowResize()` で `renderer.setSize()` は呼んでいますが、通常は **`composer.setSize()` も更新**した方が安全です（ポストプロセス解像度ずれ防止）。
- `raymarchSteps` は見た目向上に効く一方でコスト増なので、端末性能に応じて調整対象です。

---

> Improved AnaglyphEffect 
...
補足として、`planeDistance` は**シーンの単位系**（1 unit を何m相当とみなすか）に依存するため、カメラ距離やモデルスケールと合わせて調整するのが重要です。
...
- WebGPU + TSL で、**PS1風のレトロ画面**をポストプロセスで作る実装例です。
...
- **「1つのノード効果」ではなく、`retroPass` をベースに複数ノードを直列合成**している点。
- GUIでパラメータ調整しつつ、`renderPipeline.outputNode` の差し替えで **効果ON/OFF** を即時反映できる点。
- モデル切替時に retro.disposeを呼び、内部テクスチャを再生成している点（実運用で重要）。

---

> USD Kitchen set successfully loaded in the editor [https://openusd.org/release/dl_kitchen_set.html](https://t.co/CqCgWMVi0p)

---

> New animation panel in the editor 
> [https://threejs.org/editor/](https://t.co/rqspz5lp0g)

主な変更点は、**Editorの大規模刷新**です。  
...
特に「過去のフレームのピクセルを現在のフレームに再投影（Reprojection）して再利用する」という仕組み。
##### メリット
- **低負荷:** 1フレームあたりの計算量は少ないまま、実質的に多点サンプリングしたような滑らかな結果が得られます。
- **スペキュラエイリアシングに強い:** 金属などのハイライトがチカチカする現象（Shimmering）を非常に効果的に抑えられます。
- **アップスケーリングの基礎:** NVIDIAのDLSSやAMDのFSRなど、最新の超解像技術のベースとなっている技術です。
##### デメリット
- **ゴースト（残像）:** 動きの激しい物体の後ろに、過去のフレームの情報が「引きずり」として残ることがあります。
- **画面のぼやけ:** 複数のフレームを混ぜる性質上、静止画に比べて全体的に少しソフト（ぼやけた）印象になりやすいです。
    

---

## 3. なぜ今 TRAA / TAA が主流なのか

現代のゲームエンジン（Unreal Engine, Unityなど）や Three.js などのレンダラーで TAA が標準となっているのには理由があります。

- **ディファードレンダリングとの相性:** 昔主流だった **MSAA** は、現在の主流であるディファードレンダリング（遅延レンダリング）では構造上使いにくく、非常に重くなります。
    
- **レイトレーシング:** レイトレーシングなどのノイズが多い描画結果を、時間の蓄積によって「均す」役割も兼ねています。
