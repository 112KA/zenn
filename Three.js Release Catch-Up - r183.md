...
特に以下2点はコードの修正箇所が多くなりそう。
- `Clock` から より高機能な `Timer` クラスに置き換わります。[[#Timerの機能（Clockとの違い）]]
...
「静的な水面＋空」から「HDR＋ブルーム＋雲制御＋時間変化を持つ、より映画的なシーン」へ表現が拡張されました。
- **描画品質/HDRパイプラインの強化**
...
- Timerは`update()`で内部状態を更新するので1フレーム内でgetDelta()などを複数回呼んでも値が変わらない。(ClockはgetDelta()を呼ぶ毎に違う値を取得していた)
#### 何故HalfFloatTypeか
